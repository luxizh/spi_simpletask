...
wMax = 1.8 #2 for single 1.8 for multiple#1 # G: 0.15 1
...
def train(untrained_weights=None):
    ...
    #def projections
    stdp_proj = sim.Projection(layer1, layer2, sim.AllToAllConnector(), synapse_type=sim.StaticSynapse(weight=untrained_weights, delay=__delay__))
    #stdp_proj = sim.Projection(layer1, layer2, sim.FromListConnector(connections), synapse_type=sim.StaticSynapse)
    ...
    title="Single_car Random Test without Noise",
    annotations="Single_car Training and Test without Noise"
...
np.save("spi_simpletask/onesampleweight"+str(trylabel)+".npy",weight_list)

#np.save("class_result/trainedweight.npy",weight_list)
#for i in range(10):
...
weight_list=np.load("spi_simpletask/onesampleweight"+str(trylabel)+".npy")
