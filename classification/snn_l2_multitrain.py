...
aPlus = 0.0800  #tum 0.016 #9 #3 #0.5 # 0.03 from literature
aMinus = 0.0400 #255 #tum 0.012 #2.55 #2.55 #05 #0.5 # 0.0255 (=0.03*0.85) from literature 
wMax = 0.8 #1 # G: 0.15 1
wMaxInit = 0.2#0.1#0.100
wMin = 0
nbIter = 5
...
def generate_data():
    ...
    for j in range(output_size):
        labelSpikes.append([])
        multitrain=random.sample(range(40),10)
        multitest=random.sample(range(30),8)
        for k in multitest:
            labelSpikes[j].append(k*10*v_co)    
        for i in range(input_len):
            neuid=(j,i)
            for k in multitest:
                organisedData[neuid].append((k*10+i)*v_co)
            ...
            for k in multitest:
                organisedData[neuid].append(450+(k*10+i)*v_co)
...
#            organisedData[neuid].append(i*v_co)
    ...
    return [spikesTrain,labelSpikes]
'''    
